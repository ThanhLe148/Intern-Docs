...
> **Instructor note:** The ppt-visual skill should now be installed and active. Start a **NEW Codex conversation** (do not continue the old one). This is important so the skill is picked up fresh. Upload the same `quarterly_sales.csv` file again.
