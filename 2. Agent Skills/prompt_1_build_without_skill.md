...
> **Instructor note:** Before running this prompt, make sure no presentation or PPT-related skills are installed in Codex. Upload the `quarterly_sales.csv` file from the `datasets/` folder into the Cowork conversation.
...
> **Instructor note:** Without the PPT skill, Codex will produce basic slides: bullet-heavy, default fonts, charts dumped without context, no visual hierarchy, every slide looks the same, walls of text. This is the "before" for comparison. Keep the file or take screenshots for the side-by-side.
